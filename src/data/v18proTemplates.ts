...
export const V18PRO_FIXED_TAGS = "#DyMona #V18PRO #aspiradora #limpiezahogar #tiktokshop #hogarlimpio";
...
export const V18PRO_HOOK_TEMPLATES: HookTemplate[] = [
...
const V18PRO_PREFIX_PAIRS: [string, string][] = [
...
const V18PRO_SUFFIX_PAIRS: [string, string][] = [
...
export function generateV18proAlgorithmicTitles(
  ...
  const brand = "DyMona";
  const model = "V18 PRO";
  const activeTags = (customTags && customTags.trim()) ? customTags.trim() : V18PRO_FIXED_TAGS;
  const results: GeneratedTitle[] = [];
  const seenHooks = new Set<string>();

  // Filter templates by category
  let pool = V18PRO_HOOK_TEMPLATES.filter(
    (t) => category === "all_mixed" || t.category === category
  );
  if (pool.length === 0) pool = V18PRO_HOOK_TEMPLATES;
  ...
  while (results.length < 50 && attempt < 300) {
    ...
    if (styleRoll < 0.28 && !baseHook.startsWith("¡") && !baseHook.startsWith("¿")) {
      const [pfx, pfxZh] = V18PRO_PREFIX_PAIRS[Math.floor(Math.random() * V18PRO_PREFIX_PAIRS.length)];
      baseHook = `${pfx} ${baseHook}`;
      baseZh = `${pfxZh} ${baseZh}`;
    } else if (styleRoll > 0.72 && baseHook.length < 70 && !baseHook.endsWith("!") && !baseHook.endsWith("?")) {
      const [sfx, sfxZh] = V18PRO_SUFFIX_PAIRS[Math.floor(Math.random() * V18PRO_SUFFIX_PAIRS.length)];
    ...
    results.push({
      id: `v18pro-algo-${batchSeed}-${results.length + 1}-${Math.random().toString(36).substr(2, 4)}`,
      productId: "v18pro",
      title: fullTitle,
      hook: baseHook,
      tags: activeTags,
