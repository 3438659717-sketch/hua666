...
export const PetWardrobeModal: React.FC<PetWardrobeModalProps> = ({
  ...
  const handleUnlockAllCostumes = () => {
    playPetSound("levelUp");
    onUpdateState((prev) => ({
      ...prev,
      level: Math.max(prev.level, 15),
      coins: prev.coins + 888,
      unlockedAccessories: Array.from(new Set([...prev.unlockedAccessories, ...accessoryKeys])),
    }));
    onShowToast("👑 已成功解锁全衣橱所有服饰配饰！您可以尽情自由装扮萌宠！", "success");
  };
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-white/10 bg-slate-950/40">
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={handleUnlockAllCostumes}
              className="px-3 py-1.5 rounded-xl bg-gradient-to-r from-amber-500/20 to-yellow-500/20 hover:from-amber-500/30 hover:to-yellow-500/30 border border-amber-400/40 text-amber-300 text-xs font-bold flex items-center gap-1.5 transition-all shadow-sm"
              title="一键解锁所有服饰并补充金币"
            >
              <Sparkles className="w-3.5 h-3.5 text-amber-300" />
              <span>👑 一键全衣橱解锁 (VIP)</span>
            </button>
            <button
              onClick={onClose}
        ...
        <div className="p-4 bg-gradient-to-r from-slate-950/90 via-slate-900/70 to-slate-950/90 border-b border-white/10 flex flex-col sm:flex-row items-center justify-between gap-4">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs px-2.5 py-0.5 rounded-full bg-cyan-500/20 text-cyan-300 font-mono">
                  {ACCESSORY_SPRITES[previewAcc]?.name || "原生态"}
                </span>
                {previewAcc !== state.currentAccessory && (
                  <span className="text-[10px] px-2 py-0.5 rounded-full bg-purple-500/30 text-purple-200 border border-purple-400/40 animate-pulse">
                    ✨ 试穿预览中
                  </span>
                )}
              </div>
          ...
          <div className="flex flex-col sm:flex-row items-center gap-3">
            {previewAcc !== state.currentAccessory && (
              <button
                onClick={() => handleSelectAccessory(previewAcc)}
              ...
              >
                <Check className="w-4 h-4" /> 确认穿戴当前试穿服饰
              </button>
            )}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
                {accessoryKeys
                  ...
                  .map((accKey) => {
                    ...
                    const isPreviewing = previewAcc === accKey;

                    return (
                      <div
                        key={accKey}
                        onClick={() => setPreviewAcc(accKey)}
                      ...
                      >
                        <div>
                          <div className="flex items-center justify-between gap-2">
                            <div className="flex items-center gap-2">
                              <div className="w-12 h-12 rounded-lg bg-slate-950/80 border border-white/10 flex items-center justify-center overflow-hidden flex-shrink-0 shadow-inner group-hover:scale-105 transition-transform">
                                <RealisticPetCanvas
                                  species={previewPet}
                                  accessory={accKey}
                                  size={44}
                            ...
                            <div className="text-right">
                              {isEquipped ? (
                                <span className="px-2 py-0.5 rounded-full bg-purple-500 text-white text-[10px] font-bold flex items-center gap-1">
                                  <Check className="w-3 h-3" /> 穿戴中
                                </span>
                              ) : isPreviewing ? (
                                <span className="px-2 py-0.5 rounded-full bg-cyan-500/20 text-cyan-300 border border-cyan-400/40 text-[10px] font-bold animate-pulse">
                                  试穿中
                                </span>
                              ) : isUnlocked ? (
                          ...
                          <div className="mt-2.5 font-bold text-xs text-white flex items-center gap-1">
                            <span className="text-[10px] text-purple-300/80 font-normal">
                              {getCategoryLabel()}
                            </span>
                            <span>{acc.name}</span>
                          </div>
                          <p className="text-[11px] text-white/50 mt-0.5 line-clamp-2">{acc.desc}</p>
                        </div>
                          ...
                          <div className="flex items-center gap-1.5" onClick={(e) => e.stopPropagation()}>
                            {isEquipped ? (
                              <button
                                ...
                                className="px-2.5 py-1 rounded-lg bg-white/10 hover:bg-white/20 text-white/70 hover:text-white text-[11px] transition-colors"
                              >
                                卸下
                              </button>
                            ) : isUnlocked ? (
                              <button
                                ...
                                className="px-3 py-1 rounded-lg bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-500 hover:to-indigo-500 text-white text-[11px] font-bold shadow-sm transition-all"
                              >
                                佩戴
                              </button>
                            ) : (
